...
try{
		...
		$query = $conn->prepare("SELECT movierating.Likes, movielist.Title FROM `movierating` INNER JOIN movielist ON movierating.ID = movielist.ID ORDER BY `movierating`.`Likes` DESC LIMIT 10");
		// executing query
		$query->execute();
		...
		$result = $query->fetchAll(PDO::FETCH_OBJ);
			foreach($result as $row) {
				...
				array_push($array, array("y"=> $row->Likes, "label"=> $row->Title));
			}
			$conn = null;
	}
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	title:{
		text: "Top 10 Highest Ratings"
	},
	data: [{
		type: "pie",
		startAngle: 240,
		yValueFormatString: "(##0\"\" Likes)",
		indexLabel: "{label} {y}",
